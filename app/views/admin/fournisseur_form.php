<?php
$pageTitle      = ($isEdit ? 'Modifier un fournisseur' : 'Nouveau fournisseur') . ' — Administration';
$activeAdminNav = 'fournisseurs';
require VIEWS_PATH . '/layouts/admin_header.php';

$vName        = $fournisseur['name']         ?? '';
$vCategory    = $fournisseur['category']     ?? 'autre';
$vTaxId       = $fournisseur['tax_id']       ?? '';
$vContactName = $fournisseur['contact_name'] ?? '';
$vPhone       = $fournisseur['phone']        ?? '';
$vEmail       = $fournisseur['email']        ?? '';
$vVille       = $fournisseur['ville']        ?? '';
$vAddress     = $fournisseur['address']      ?? '';
$vLogoPath    = $fournisseur['logo_path']    ?? null;
$vStatus      = $fournisseur['status']       ?? 'actif';
$vNotes       = $fournisseur['notes']        ?? '';
?>
      ...
      <div class="col-lg-7">
        <div class="admin-card">
          <div class="admin-card-header">
            <h2><i class="bi bi-truck"></i> <?= $isEdit ? 'Modifier le fournisseur' : 'Nouveau fournisseur' ?></h2>
          </div>
          ...
          <div class="p-3">
            ...
            <form method="post" action="<?= $isEdit ? url('/admin/fournisseurs/modifier?id=' . (int) $fournisseur['id']) : url('/admin/fournisseurs/nouveau') ?>" enctype="multipart/form-data">
              <div class="mb-3">
                <label class="form-label-mini" for="name">Nom du fournisseur</label>
                <input type="text" class="form-control-auth" id="name" name="name" value="<?= e($vName) ?>" placeholder="Ex : Imprimerie Nationale d'Haïti" required>
              </div>

              <div class="row g-3 mb-3">
                <div class="col-6">
                  <label class="form-label-mini" for="category">Catégorie</label>
                  <select class="form-control-auth" id="category" name="category">
                    <?php foreach ($categoryLabels as $key => $label): ?>
                      <option value="<?= e($key) ?>" <?= $vCategory === $key ? 'selected' : '' ?>><?= e($label) ?></option>
                    <?php endforeach; ?>
                  </select>
                </div>
                <div class="col-6">
                  <label class="form-label-mini" for="tax_id">NIF / Matricule</label>
                  <input type="text" class="form-control-auth" id="tax_id" name="tax_id" value="<?= e($vTaxId) ?>" placeholder="Ex : NIF-0001-IMP">
                </div>
              </div>

              <div class="mb-3">
                <label class="form-label-mini" for="contact_name">Nom du contact / responsable</label>
                <input type="text" class="form-control-auth" id="contact_name" name="contact_name" value="<?= e($vContactName) ?>" placeholder="Ex : Service Production">
              </div>
              ...
              <div class="mb-3">
                <label class="form-label-mini" for="logo">Logo du fournisseur</label>
              ...
              <div class="admin-form-actions">
                <button type="submit" class="mini-btn flex-fill"><i class="bi bi-check-lg"></i> <?= $isEdit ? 'Enregistrer' : 'Créer le fournisseur' ?></button>
                <a href="<?= url('/admin/fournisseurs') ?>" class="mini-btn-outline">Annuler</a>
              </div>
            </form>
          </div>
        </div>
        ...
        <div class="admin-card">
          ...
          <div class="p-4 d-flex flex-column align-items-center gap-3" id="previewBox">
            ...
            <div style="text-align:center;">
              <div style="font-weight:700;color:var(--bleu-nuit);" id="previewName"><?= $vName !== '' ? e($vName) : 'Nom du fournisseur' ?></div>
              <div style="font-size:.8rem;color:var(--gris-texte);" id="previewCategory"><?= e($categoryLabels[$vCategory] ?? $vCategory) ?></div>
            </div>
          ...
          <p style="color:var(--gris-texte);font-size:.78rem;padding:0 1.2rem 1.2rem;text-align:center;">
            Aperçu de la fiche telle qu'elle apparaîtra dans le dossier des fournisseurs.
          </p>
        </div>
      </div>
    </div>
...
<script>
// ---------- Aperçu en direct du formulaire "Fournisseurs" ----------
...
function refreshPreview(){
  previewName.textContent = nameEl.value.trim() || 'Nom du fournisseur';
  previewCategory.textContent = categoryEl.options[categoryEl.selectedIndex].text;
...
[nameEl, categoryEl, statusEl].forEach(el => {
